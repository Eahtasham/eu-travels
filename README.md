...
A premium travel-agency landing page built around a scroll-scrubbed hero — as you scroll, the page plays through a plane-window image sequence: cabin window → clouds → an emergent view of the Taj Mahal above the clouds.

**Live demo:** run locally with the steps below.

## Highlights

- **Scroll-scrubbed image-sequence background.** A `<canvas>` preloads a 306-frame sequence and paints the frame nearest the eased, scroll-derived playhead. Every seek is an instant image swap — no video-decode/keyframe latency — with a `prefers-reduced-motion`-safe fallback.
...
- **Thin glass surfaces.** Cards are a light frosted layer over the background, not an opaque wall, so the scene stays part of the experience throughout every section.
...
src/
  ...
  components/
    ScrollCanvas.tsx   Scroll-scrubbed image-sequence background + loader
    Reveal.tsx          Visible-by-default scroll-reveal wrapper
public/
  plane-window/         306-frame JPG sequence for the scroll background
```
